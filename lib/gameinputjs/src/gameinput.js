...
class GameInput {
  ...
  constructor (options = undefined) {
    ...
    if (GameInput.canUseGamepadAPI()) {
      if (this.debug) { console.debug('Gamepad connection loop beginning') }
  ...
  buttonDown (player, sectionName, buttonName) {
    for (const action in this.buttonDownActions) {
      if (typeof (this.buttonDownActions[action]) === 'function') { this.buttonDownActions[action](player, sectionName, buttonName) }
    }
    return this
  }
  ...
  buttonUp (player, sectionName, buttonName) {
    for (const action in this.buttonUpActions) {
      if (typeof (this.buttonUpActions[action]) === 'function') { this.buttonUpActions[action](player, sectionName, buttonName) }
    }
    return this
  }
   ...
   */
  getPlayer (index) {
    if (index < 0 || index > 3) { throw new Error('Index out of the 0-3 range!') }
   ...
   */
  getGamepad (player) {
    if (player < 0 || player > 3) { throw new Error('Index out of the 0-3 range!') }
   ...
   */
  #nextUpdateLoop () {
    if (this.#updateActive === false) { return }
   ...
   */
  update () {
    if (!GameInput.canUseGamepadAPI()) { return }
    ...
    for (let i = 0; i < this.Connection.Gamepads.length; i++) {
      ...
      if (typeof (currentGamepad) === 'undefined' || currentGamepad === null || !player.model) { continue }
  ...
  connectionWatchLoop () {
    ...
    if (this.lastCheckedNumberOfGamepads !== currentNumberOfGamepads) {
      if (this.debug) { console.debug('Now have ' + currentNumberOfGamepads + ' gamepad(s).') }
  ...
  reinitialize () {
    ...
    for (const player of this.Players) { player.setModel(undefined) }

    if (GameInput.canUseGamepadAPI()) {
      this.Connection.Gamepads = navigator.getGamepads()

      for (let i = 0; i < this.Players.length; i++) {
        if (this.Connection.Gamepads.filter(Boolean).length === 0) { this.#firstPress[i] = false }
      }
