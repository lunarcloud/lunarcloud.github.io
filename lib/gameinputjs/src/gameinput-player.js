...
export class GameInputPlayer {
  ...
  buttonDown (sectionName, buttonName) {
    ...
    if (actionList) { for (const action of actionList) action() }
  }
  ...
  onButtonDown (sectionName, buttonName, action) {
    ...
    if (!this.buttonDownActions.has(`${sectionName}.${buttonName}`)) { this.buttonDownActions.set(`${sectionName}.${buttonName}`, []) }
  ...
  onButtonUp (sectionName, buttonName, action) {
    ...
    if (!this.buttonUpActions.has(`${sectionName}.${buttonName}`)) { this.buttonUpActions.set(`${sectionName}.${buttonName}`, []) }
  ...
  getStickVector (stick) {
    if (stick !== 'left' && stick !== 'right') { throw new Error('Must be left or right') }
  ...
  getNormalizedTriggerValue (trigger) {
    if (trigger !== 'left' && trigger !== 'right') { throw new Error('Must be left or right') }

    const item = this.state.trigger[trigger]

    if (typeof (item.value) === 'number') { return item.active ? 1 : 0 } else if (item.item instanceof AxisAsButton) {
