...
export default class BgAudioManager {
  ...
  constructor () {
    ...
    if (bgAudioEl instanceof HTMLAudioElement) { this.bgAudio = bgAudioEl } else { throw new Error("BG Audio isn't an audio element!") }
  ...
  pageVisibilityChanged () {
    ...
    if (document.visibilityState === 'visible') { this.bgAudio.play() } else { this.bgAudio.pause() }
  }

  /**
   * @callback SetupAction  Callback for setup actions
   ...
   * @param {SetupAction} onHover Element hovering handler.
   * @param {SetupAction} onLeave Element un-hovering handler.
   * @param {SetupAction} onClick Element clicking handler.
   */
  setupElements (elements, onHover, onLeave, onClick) {
    if (typeof elements === 'string') { elements = document.querySelectorAll(elements) }

    if (!(elements?.[0] instanceof Element)) { throw new Error('Error during setup, can\'t use these elements') }
