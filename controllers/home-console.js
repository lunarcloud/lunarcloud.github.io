...
export default class HomeConsolePageController {
  ...
  navigate (direction) {
    ...
    switch (direction) {
      ...
      case 'left':
      {
        if (this.currentFocus.column > 0) { this.setFocus(this.currentFocus.row, this.currentFocus.column - 1) }
        break
      }
      case 'right':
      {
        if (this.currentFocus.column < this.focusableElements[this.currentFocus.row].length - 1) { this.setFocus(this.currentFocus.row, this.currentFocus.column + 1) }
        break
      }
    }
  }
  ...
  activateMenu () {
    ...
    if (!document.body.classList.contains('paused')) { this.audioManager.playOnce(this.chimeAudio) } else { this.audioManager.playOnce(this.cancelAudio) }
  ...
  accept (fromGamepad = false) {
    ...
    if (currentEl && 'click' in currentEl) {
      currentEl.click()
      if (fromGamepad) { this.gameInput.getPlayer(0).rumble({ duration: 200, weakMagnitude: 0.0, strongMagnitude: 0.2 }) }
    }
  }
