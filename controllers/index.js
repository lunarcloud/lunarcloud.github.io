 ...
 */
function nagIfTargetOffscreen (nagArrowEl, targetEl) {
  if (isProperVisible(targetEl)) { nagArrowEl.classList.remove('bounce') } else { nagArrowEl.classList.add('bounce') }
}
 ...
 */
function isProperVisible (el) {
  const percentTop = percentDownPage(el)

  if (scrolledToBottom() && percentTop < 1) { return true }
...
document.addEventListener('scroll', () => nagIfTargetOffscreen(nagArrowEl, targetEl))
window.addEventListener('resize', () => nagIfTargetOffscreen(nagArrowEl, targetEl))
